...
begin
  ...
  if ASASL.ServiceName='NTLM' then  //NTML authention , add if contidion by lwgboy ,//已经编码过了，不再编码发送
    AClient.SendCmd(S)   //已经编码过了，不用再编码而直接发送
  else
  ...
  while PosInStrArray(AClient.LastCmdResult.Code, AContinueReplies) > -1 do begin
    ...
    if ASASL.ServiceName='NTLM' then //NTML authention , add if contidion by lwgboy ,此时AClient.LastCmdResult.Text.Text为已经解过码了的内容
      S := Sys.TrimRight(AClient.LastCmdResult.Text.Text) //add by lwgboy,已经解过码了，不用再解码，否则再次解码后为乱码，可能导致出错或出现非预期的结果而不能通过NTLM认证。
    else
      S := ADecoder.DecodeString(Sys.TrimRight(AClient.LastCmdResult.Text.Text));

    S := ASASL.ContinueAuthenticate(S);

    if ASASL.ServiceName='NTLM' then //NTML authention ,lwgboy ,在调用ASASL.ContinueAuthenticate的过程中已经编码过了，不可再次编码，而是直接发送。
      AClient.SendCmd(S)  //add by lwgboy,已经编码过了，不再编码而直接发送
    else
      AClient.SendCmd(AEncoder.Encode(S));
