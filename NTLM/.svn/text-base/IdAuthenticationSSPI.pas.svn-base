...
}
{
  ...
  Rev 1.2    13.1.2004 ? 17:26:06  DBondzhev
...
begin
  ...
  SecInvalidateHandle(@fHandle);  //Change by lwgboy.
end;
  ...
  case FCurrentStep of
    1:
      begin
        if Length(Username) = 0 then
          FSSPIClient.SetCredentialsAsCurrentUser //创建信用字符串
        else
          FSSPIClient.SetCredentials(Domain, Username, Password); //创建包含客户端机器名、登陆服务器的用户名及登陆服务器的对应密码的加密信用证书。
    ...
    2:
      begin
        ...
        begin
          {
          //注释掉下面二句,直接替换为后面一句。将SMTP Server 返回的NTLM认证信息直接赋值给FNTLMInfo变量。 --lwgboy
          FNTLMInfo := ReadAuthInfo('NTLM');  //Do not translate;
          Fetch(FNTLMInfo);
          }
          FNTLMInfo := FAuthParams[0];  //SMTP、TCP/IP等非HTTP/Web方式，Server端返回的信息头通常只有一个:NTLM AUTH 认证字符串，所以这里直接用FAuthParams[0]表示NTLM Authencation String; 而HTTP/Web方式的NTLM认证通常有多个认证头(即FAuthParams.Count>0),此时需要通过 ReadAuthInfo('NTLM') 函数来分离出真正的NTLM认证信息。          
        end;
        ...
        S := DecodeString(TIdDecoderMIME,FNTLMInfo); //解码Server端返回的NTLM Challenge 字符串
        result := 'NTLM ' + EncodeString(TIdEncoderMIME,FSSPIClient.UpdateAndBuildType3Message(S));  {Do not translate} //将客户端对 Server 端发回的 NTLM Challenge 字符串 之 响应结果字符串编码。
        FCurrentStep := 3;
      end;
    3: begin
        FCurrentStep := 4;
