...
Function QuotedPrintableEncode(mSource: AnsiString): AnsiString;
Function QuotedPrintableDecode(mCode: AnsiString): AnsiString;
Function Base64Encode(mSource: AnsiString; mAddLine: Boolean = True): AnsiString;
Function Base64Decode(mCode: AnsiString): AnsiString;
Function GetTitle(Const Value: AnsiString): AnsiString;
...
Begin
  ...
  While I <= L Do Begin
      If mCode[I] = '=' Then Begin
          Result := Result + PAnsiChar(Chr(StrToIntDef('$' + Copy(mCode, I + 1, 2), 0)));
          Inc(J, 3);
          Inc(I, 3);
...
Function Find(SubStr, Str: AnsiString; IsEnd: Boolean = False): Integer;
Var
  i: integer;
Begin
  Result := 0;
