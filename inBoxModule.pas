    ...
    begin
      ...
      begin
         ...
         begin
           ...
           AsniTemp := str1List[i];
           bytesLength := DCPBase64.Base64Decode(@AsniTemp[1],@bytes,Length(AsniTemp));
           strTemp := TEncoding.UTF8.GetString(TBytes(bytes));  //这里直接处理UTF8编码，以防止Base64解码后的UTF8编码末尾可能会有的：由#和数字组成的半个字符赋给String类型变量(然后再进行UTF8解码)时，末尾的半个字符被截除。
         end
         else
           strTemp := DCPbase64.Base64DecodeStr(str1List[i]);
      end
...
begin
{
  ...
  begin
    ...
    currentPage := Request.QueryFields.Values['p'];
    if currentPage='' then currentPage := '1';
    currentPage := paginationShow(POP3,Request.InternalScriptName+Request.InternalPathInfo,Request.InternalScriptName+'/detail',WebContext.Session.SessionID+loginModule.AppendUserInfo,StrToInt(currentPage),TConfig.pageItemCount,POP3.CheckMessages);
    content := '<?xml version="1.0"?><!DOCTYPE wml PUBLIC "-//WAPFORUM//DTD WML 1.1//EN" "http://www.wapforum.org/DTD/wml_1.1.xml">';
    content := content + '<wml><card id="card1" title="'+TXMLConfig.GetAppSettingTitle+'" newcontext="true"><p>';
...
//  end;
  ...
  if pageItemNum>TConfig.pageItemCount then pageItemNum := TConfig.pageItemCount;
  ...
  begin
    ...
    begin
      ...
      content.Add('<option value="'+ IntToStr(currentPageFirstItem+i-1)+'">'+{DateTimeToStr(msg.Date,format)}FormatdateTime('yy-mm-dd hh:mm',msg.Date)+'</option>');
      content.Add('</select><br/>');
    end
    else
    begin
      content.Add(FormatdateTime('yyyymmdd hh:mm',msg.Date));
    end;
  ...
  content.Add(('共'+IntToStr(summaryPage)+'页')+',转<input name="go" maxlength="5" size="3" format="*N" emptyok="false" value="'+IntToStr(currentPage)+'"/>'+('页')+' <a href="'+WebContext.Request.InternalScriptName+'/inBox?ms='+WebContext.Session.SessionID+loginModule.AppendUserInfo+'&amp;p=$go">Go!</a>');
