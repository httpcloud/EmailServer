...
type
  TUserMailInBoxDAO = class(TMailBoxConfigDatabase)
     ...
     //读取收件箱或发件箱或草稿箱
     function ReadMailBoxBy(UserEMail:string;KeyFieldName:string;kv:TKeyValue): TObjectList<TUserMailInBoxRec>;
...
function TUserMailInBoxDAO.ReadMailBoxBy(UserEMail:String; KeyFieldName: string; kv: TKeyValue): TObjectList<TUserMailInBoxRec>;
var
  sql,time:string;
  stmt:TDISQLite3Statement;
  InBoxList:TObjectList<TUserMailInBoxRec>;
  ...
  fs:TFormatSettings;
begin
  sql := 'select * from UserInBox where UserEMail='''+UserEMail+''' and '+KeyFieldName+'=? order by RecvTime desc;';
  objList := TObjectList<TKeyValue>.Create;
  objList.Add(kv);
  stmt := inherited Read(sql,objList);
  if(stmt<>nil)then
