...
{ TWebResponse }
  ...
  protected
    ...
    //add by lwgboy
    function GetSessionID2URL: string; virtual; abstract;
    procedure SetSessionID2URL(const AValue: string); virtual; abstract;
  public
    ...
    //add by lwgboy,将SessionID保存在URL中，以适应不支持Cookie的浏览器。
    property SessionID2URL:string read GetSessionID2URL write SetSessionID2URL;
  end;
...
procedure ExtractHTTPFields(Separators, WhiteSpace: TSysCharSet; const Content: AnsiString;
  Strings: TStrings; StripQuotes: Boolean = False); overload;
...
procedure ExtractHeaderFields(Separators, WhiteSpace: TSysCharSet; const Content: AnsiString;
  Strings: TStrings; Decode: Boolean; StripQuotes: Boolean = False); overload;
...
{$IFDEF MSWINDOWS}
  Windows,
{$ENDIF} 
  WebConst, BrkrConst, StrUtils;

var
  FContentParsers: TList;  //代码解读： 用于存放所有的 ContentParser -- lwgboy, 2009-9-8 22:35

function ContentParsers: TList;     //代码解读：重要！用于存放所有的 ContentParser -- lwgboy, 2009-9-8 22:35
begin
  if FContentParsers = nil then
    FContentParsers := TClassList.Create;  //代码解读： 用于存放所有的 ContentParser -- lwgboy, 2009-9-8 22:35
...
begin
  ...
  begin
    ...
    begin
      ...
      if C.CanParse(Self) then                       //代码解读，这里通过每个 ContentParser 自己的 CanParser 判断是否属于该类型的 ContentParser，进而判断是否可以用该 ContentParser 进行解析。   --lwgboy ,2009-9-8; 我们的这里主要处理客户端通过 Wap 页面输入的并提交过来的数据( HTTPEncode(UTF8编码格式)),所以用默认的
      begin
...
begin
  ...
  if (FDefault and DoDefault) or (FEnabled and ((FMethodType = mtAny) or
    ...
    GetMask.Matches(string(Request.InternalPathInfo))) then
  begin
...
begin
  ...
  begin
    S := AnsiUpper(PAnsiChar(AContentType));
    P := PAnsiChar(S);
    P := AnsiStrPos(P, 'CHARSET'); {do not localize}
    if P <> nil then
      if (AnsiStrPos(P, 'UTF-8') <> nil) then {do not localize}  //注意这个地方，也是一个处理字符编码的地方.  --lwgboy
...
begin
  ExtractHeaderFields(Separators, WhiteSpace, PChar(string(Content)), Strings, Decode, StripQuotes);
end;
...
begin
  ...
  repeat
    ...
    begin
      ...
      if Decode then
        //Strings.Add(string(HTTPDecode(AnsiString(DoStripQuotes(ExtractedField))))) //改为下面一句，适应发送邮件时，客户端提交过来的 UTF8 编码的中文字符。-- commented by lwgboy 2009-09-08 01:40
        //Strings.Add(string(HTTPDecode(String(DoStripQuotes(ExtractedField))))) //modify by --lwgboy 2009-09-08 01:40 ,经反复测试，依然不可以处理，所以再次注释掉并继续还原为上面一句。   --lwgboy 2009-09-08 02:00
        Strings.Add(string(HTTPDecode(AnsiString(DoStripQuotes(ExtractedField)))))  //后经测试无法处理 客户端提交过来的 HTTPEncode(中文UTF8编码) 不是这里的问题，引入李伟博客上提供的 UTF8ContentParser.pas 后解决(参见SendModule.pas 之 uses UTF8ContentParser)。 --lwgboy 2009-09-08 21:30
