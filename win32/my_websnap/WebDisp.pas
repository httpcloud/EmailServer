...
type
  ...
  protected
    ...
    function GetSessionID2URL: string; override;
    procedure SetSessionID2URL(const AValue: string); override;
...
var
  ...
  FindOptions: TFindPageOptions;
begin
  { TODO -oJMT -cMUSTFIX : Allow nil AParams - Logged 115969}
  AppModule := WebContext.FindApplicationModule(nil);
  if Supports(IUnknown(AppModule), IGetWebAppComponents, GetIntf) then
    if GetIntf.GetPageDispatcher <> nil then
    begin
...
begin
  ...
  { TODO -oJMT -cMUSTFIX : roExcludeID no longer used }
  if Identifier <> '' then
...
begin
  { TODO -oJMT -cMUSTFIX : 
Create the thread variable first so that the sessions object can get at the
request to look for a session value there - Logged 115971}
...
var
  ...
  HaveID: Boolean;
  UserAndPassword:string;  //重要，保存用户名及密码
begin
  ...
  begin
    ...
    {//将系统下面的这段话注释掉改为下面的一段，以适应wapmail..cc自动登录的需求，免除每次都需要输入用户名、密码。  --lwgboy 2008-2-21
    if FWebSession = nil then
      FWebSession := ASessionsService.NewSession;
    }

    //add by lwgboy,2008-2-21，为了根据用户保存的URL(内含加密的用户名和密码)来自动登录系统。
    if FWebSession = nil then
    begin
      FWebSession := ASessionsService.NewSession;
      if HaveID then
      begin
        ...
        begin
          UserAndPassword := ARequest.QueryFields.Values['up'];
          if UserAndPassword = '' then
             UserAndPassword := ARequest.QueryFields.Values['amp;up'];   //&符号有时会被转换成&amp; 此时取up就取不到了，而必须以 amp;up 为名字来取 --lwgboy ,2009-9-13

          exit;
          if UserAndPassword<>'' then //up:UserName and Password的缩写
            AResponse.SendRedirect(ARequest.InternalScriptName+'/login?up='+UserAndPassword);
        end;
      end;
    end;
...
begin
  ...
  //add by lwgboy, 为了支持不支持Cookie的浏览器，将SessionID保存在URL中，提交过来。
  if (ARequest.QueryFields.Values['ms'] <> '') then //ms:MailSession
  begin
     Result := True;
     AID := ARequest.QueryFields.Values['ms'];
  end
  else
  if (ARequest.ContentFields.Values['ms'] <> '') then //当点击WML中的Do按钮提交时，其SessionID虽然是通过URL后缀表示的，但实际检测发现它并未通过QueryFields传过来，而是同内容一道通过ContentField传过来的，所以又加了条件（ARequest.ContentFields.Values['ms'] <> ''）来判断并取出SessionID.
  begin
     Result := True;
     AID := ARequest.ContentFields.Values['ms'];
  end
  else
    Result := False;
  { TODO -oJMT -cMUSTFIX : Why do we initialize the session cookie to blank.  How about updating it using the current session id }
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        { TODO -oJMT -cMUSTFIX : What is this code doing? }
...
//add by lwgboy,将SessionID保存在URL中，以适应不支持Cookie的浏览器。
function TWebSession.GetSessionID2URL:string;
var
  Intf: IWebSessionAttributes;
begin
  Result := '';
  if Assigned(FWebSession) then
    if Supports(FWebSession, IWebSessionAttributes, Intf) then
      Result := Intf.SessionID2URL;
end;

//add by lwgboy,将SessionID保存在URL中，以适应不支持Cookie的浏览器。
procedure TWebSession.SetSessionID2URL(const AValue: string);
var
  Intf: IWebSessionAttributes;
begin
  if Assigned(FWebSession) then
    if Supports(FWebSession, IWebSessionAttributes, Intf) then
      Intf.SessionID2URL := AValue;
end;
